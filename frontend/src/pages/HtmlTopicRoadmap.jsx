...
import htmlLearningPath from "../data/htmlLearningPath"
...
export default function HtmlTopicRoadmap() {
  const { topicId } = useParams()
  const [openPhaseIds, setOpenPhaseIds] = useState([])
  const [openSectionIds, setOpenSectionIds] = useState([])

  const topic = htmlLearningPath.topics.find(item => item.id === topicId)
  const content = topic ? htmlLearningPath.content[topic.id] : null
  ...
  if (!content) {
    return (
      <div className="max-w-7xl mx-auto p-4 md:p-10 text-gray-900 dark:text-white transition-colors">
        ...
        <Link to="/learning-paths/web-development" className="text-cyan-500 dark:text-cyan-300 hover:underline mt-4 inline-block transition-colors">
          Back to Web Development topics
        </Link>
      </div>
    )
  }
                              ...
                              <div className="space-y-1 px-3 md:px-4 pb-4 pt-1">
                                {section.points.map((point, pointIndex) => (
                                  <Link
                                    ...
                                    to={`/web-development/${topic.id}/${point.id}`}

...
import { aiMlTopicsContent } from "../data/aiMlTopicsContent"

function renderExplanationWithLinks(text) {
  const urlPattern = /(https?:\/\/[^\s]+)/g

  return text.split("\n").map((line, lineIndex) => (
    <span key={`line-${lineIndex}`}>
      {line.split(urlPattern).map((chunk, chunkIndex) => {
        const isUrl = /^https?:\/\/[^\s]+$/.test(chunk)
        if (!isUrl) {
...
export default function AiMlPointDetail() {
  const { topicId, pointId } = useParams()
  const content = aiMlTopicsContent[pointId]
  ...
  if (!content) {
    return (
      <div className="p-4 md:p-10 text-gray-900 dark:text-white transition-colors">
        ...
        <Link to={`/ai-ml/${topicId}`} className="text-cyan-400 hover:underline mt-4 inline-block">
          Return to Roadmap
        </Link>
      </div>
    )
        ...
        <div className="space-y-4">
            ...
            {isSectionOpen("explanation") && (
              <div className="px-4 pb-4">
                <p className="text-gray-700 dark:text-gray-300 text-base md:text-lg leading-relaxed transition-colors">
                  {renderExplanationWithLinks(content.explanation)}
                </p>
              </div>
            )}
          </section>
          ...
          {content.visual && (
            <section className="rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/30 transition-colors">
              ...
              {isSectionOpen("visual") && <div className="px-4 pb-4">{content.visual}</div>}
            </section>
          )}
          ...
          {content.code?.python && (
            <section className="rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/30 transition-colors">
              <button
                ...
                className="w-full flex items-center justify-between gap-4 px-4 py-3 text-left"
              >
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white transition-colors">Python Snippet</h2>
                <span className="shrink-0 rounded-md border border-gray-300 dark:border-gray-600 px-2 py-1 text-xs font-semibold text-gray-600 dark:text-gray-300 transition-colors">
